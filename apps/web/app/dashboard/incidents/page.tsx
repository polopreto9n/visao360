  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900">Incidentes</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-16 text-center">
          ...
          <p className="text-lg font-semibold text-slate-700">Nenhum incidente registrado</p>
        </div>
      ) : (
      ...
      {/* Modal criar */}
      <Modal open={creating} onClose={() => setCreating(false)} title="Registrar Incidente" size="md">
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div>
        ...
        <textarea required rows={4} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm resize-none focus:ring-2 focus:ring-blue-500 outline-none"
          value={form.description} onChange={(e) => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Descreva o incidente em detalhes..." />
      </div>
      ...
      <button type="submit" disabled={saving}
        ...
        {saving ? 'Registrando...' : '⚠️ Registrar Incidente'}
      </button>
    </form>
  );
}
